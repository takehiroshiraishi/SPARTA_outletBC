...
#include "math.h"
#include "surf_collide_outlet_vgrad.h"
#include "surf.h"
#include "input.h"
#include "modify.h"
#include "comm.h"
...
SurfCollideOutletVgrad::SurfCollideOutletVgrad(SPARTA *sparta, int narg, char **arg) :
  SurfCollide(sparta, narg, arg)
{
  if (narg != 8 ) error->all(FLERR,"Illegal surf_collide outlet_vgrad command");
...
SurfCollideOutletVgrad::~SurfCollideOutletVgrad()
{
  if (copy) return;
...
Particle::OnePart *SurfCollideOutletVgrad::
        ...
        int isurf, double *norm, int isr, int &reaction)
{
  ...
  double u0dottan = u0[0]*tangent1[0] + u0[1]*tangent1[1] + u0[2]*tangent1[2];
